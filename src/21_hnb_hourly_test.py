"""
HNB 1-HOUR bar forecast: ARIMA, LSTM, Hybrid (+ Naive), one-step & multi-step.

Same methodology as the daily tests, but on 1-hour bars (7,528 bars, 2019-2026,
TradingView OLHC). Unlike the daily HNB series (88 pts), this finally gives the LSTM
enough data to train properly. Univariate on `close`; bars treated as a sequential
series (overnight gaps ignored, like trading days). 80/20 chronological split.

Artifacts:
  results/figures/hnb_hourly_overlay.png
  results/tables/hnb_hourly_comparison.csv
"""
...
def lstm_recursive(m, seed_win, steps, scaler):
    m.eval(); win = list(seed_win); out = []
    with torch.no_grad():
        for _ in range(steps):
            x = torch.tensor(np.array(win[-WIN:], np.float32)).reshape(1, -1, 1); p = m(x).item(); out.append(p); win.append(p)
    return scaler.inverse_transform(np.array(out).reshape(-1, 1)).ravel()


# ---- data ----
df = pd.read_csv(DATA / "hnb_hourly.csv", parse_dates=["datetime"]).sort_values("datetime").reset_index(drop=True)
...
d_te = dts.iloc[split:]
full = prices
print(f"HNB 1-hour | {n} bars | Train {split} ({dts.iloc[0]} -> {dts.iloc[split-1]}) | "
      f"Test {n-split} ({d_te.iloc[0]} -> {d_te.iloc[-1]})")
print(f"Test close moved {p_te[0]:.1f} -> {p_te[-1]:.1f} ({(p_te[-1]-p_te[0])/p_te[0]*100:+.1f}%)")
...
lstm_ms = lstm_recursive(m, sct[-WIN:], n-split, sca)
...
mr = train_lstm(scrt); mr.eval(); nrt = len(res_tr)
with torch.no_grad():
    Xt = np.array([scrf[i-WIN:i] for i in range(nrt, len(fr))], np.float32)
    hyb_res_1s = rs.inverse_transform(mr(torch.tensor(Xt).unsqueeze(-1)).numpy()).ravel()
res_tr_ms = (p_tr - np.exp(ARIMA(lg, order=o2).fit().fittedvalues.values))[1:]
rs2 = MinMaxScaler((0, 1)); scr2 = rs2.fit_transform(res_tr_ms.reshape(-1, 1)).ravel()
mr2 = train_lstm(scr2)
hyb_res_ms = lstm_recursive(mr2, scr2[-WIN:], n-split, rs2)
res["Hybrid"] = {"1s": arima_1s + hyb_res_1s, "ms": arima_ms + hyb_res_ms}
...
tbl = pd.DataFrame(rows); tbl.to_csv(TAB / "hnb_hourly_comparison.csv", index=False)
print(f"\nARIMA order 1s{o1} ms{o2} | LSTM window {WIN}\n")
print(tbl.round(3).to_string(index=False))

# plot (lines, no markers — too many points)
fig, (a1, a2) = plt.subplots(1, 2, figsize=(16, 6))
for ax, proto, title in [(a1, "1s", "One-step-ahead (next hour)"), (a2, "ms", "Multi-step (forecast whole test window)")]:
    ax.plot(d_te, p_te, color="black", lw=1.5, label="Actual")
    ax.plot(d_te, res["ARIMA"][proto], color="tab:blue", lw=1, alpha=0.8, label="ARIMA")
    ax.plot(d_te, res["LSTM"][proto], color="tab:green", lw=1, alpha=0.8, label="LSTM")
    ax.plot(d_te, res["Hybrid"][proto], color="tab:red", lw=1, alpha=0.8, label="Hybrid")
    ax.plot(d_te, res["Naive"][proto], color="gray", lw=1, ls=":", label="Naive")
    ax.set_title(title); ax.set_ylabel("HNB close (LKR)"); ax.grid(True, alpha=0.3); ax.legend()
fig.suptitle(f"HNB 1-hour bars: ARIMA / LSTM / Hybrid vs actual (test {d_te.iloc[0].date()} -> {d_te.iloc[-1].date()})", fontsize=13)
fig.tight_layout(); fig.savefig(FIG / "hnb_hourly_overlay.png", dpi=150); plt.close(fig)
print("\nSaved: hnb_hourly_overlay.png, hnb_hourly_comparison.csv")
